...
import { GET_MITRA_ALL_BY_FASILITATOR, GET_MITRA_ALL_BY_SU_YES } from '../../../api/mitra';
import { GET_PEMBELIAN_MITRA_PERBULAN, GET_PEMBELIAN_MITRA_PERBULANLINE, GET_PEMBELIAN_MITRA_PERBULANLINECONTINUES, GET_PEMBELIAN_MITRA_PERBULANLINECONTINUESFASILITATOR, GET_PENJUALAN_MITRA_PERBULANLINE, GET_PENJUALAN_MITRA_PERBULANLINECONTINUES } from '../../../api/report';
import AutoCompleteLoading from '../../../components/AutoCompleteLoading';
import { yearOption } from '../../../utils/yearOption';

const tahunSekarang = new Date().getFullYear();

export default function PembelianPerMitraPerbulanLineContinue({ type = 'line' }) {
  ...
  const handleFilter = () => {
    refetch(tahun?.value, mitraCode?.value, sessionStorage.getItem("kordinator"));
    setOpen(false);
  };

  const { data, isLoading, refetch } = useQuery(['GET_BELIAN_MITRA_PERBULANLINECONTINUFASILITATOR', tahun?.value], () =>
  GET_PEMBELIAN_MITRA_PERBULANLINECONTINUESFASILITATOR(tahun?.value, mitraCode?.value, sessionStorage.getItem("kordinator"))
  );
  const { data: mitraData } = useQuery('GET_MITRA_ALL_BY_SU_YES', GET_MITRA_ALL_BY_FASILITATOR);
  ...
  // if (type === 'line') {
    ...
    state = {
      ...
      options: {
        ...
        title: {
          text: 'TOTAL PEMBELIAN PERBULAN TERKUMPUL DARI AWAL BERGABUNG ',
          align: 'left',
        },
        grid: {
          row: {
