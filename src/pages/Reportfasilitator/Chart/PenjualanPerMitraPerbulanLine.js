...
import { GET_PEMBELIAN_MITRA_PERBULAN, GET_PEMBELIAN_MITRA_PERBULANLINE, GET_PENJUALAN_MITRA_PERBULANLINE, GET_PENJUALAN_MITRA_PERBULANLINEFASILITATOR } from '../../../api/report';
...
export default function PenjualanPerMitraPerbulanLine({ type = 'line' }) {
  ...
  const { data, isLoading, refetch } = useQuery(['GET_PENJUALAN_MITRA_PERBULANLINEFASILITATOR', tahun?.value], () =>
  GET_PENJUALAN_MITRA_PERBULANLINEFASILITATOR(tahun?.value, mitraCode?.value, sessionStorage.getItem("kordinator"))
  );
  const { data: mitraData } = useQuery('GET_MITRA_ALL_BY_SU_YESFS', GET_MITRA_ALL_BY_FASILITATOR);
