import { Box, Button, CardContent, Container, Drawer, List, Stack, TextField, Typography } from '@mui/material';
...
import { GET_ALL_ANGGOTA, GET_ALL_ANGGOTA_BY_WILAYAH, GET_MAP_ANGGOTA } from '../../api/dashboard/index.js';
...
export default function PetaSebaran() {
  ...
  const warna = new Array('#F00', '#ffff00', '#005187', '#155612', '#ff00d0', '#0cd151', '#000');
        ...
        <MapContainer style={{ height: '546px', width: '100%' }} center={center} zoom={10} scrollWheelZoom={false}>
          ...
          >
            ...
            {list &&
              list?.map((a, i) => {
                return (
                  <h6 style={{ color: warna[i] }} key={i}>{a?.nama?.nama}</h6>
                );
          ...
          {list &&
            list?.map((a, i) => {
              var color = warna[i];
              var mitra = '<svg version="1.1" id="marker-11" xmlns="http://www.w3.org/2000/svg"  fill=' + color + ' fill-opacity="1" width="20px" height="20px" viewBox="0 0 11 11"><path id="path4133" d="M5.5-0.0176c-1.7866,0-3.8711,1.0918-3.8711,3.8711&#xA;&#x9;C1.6289,5.7393,4.6067,9.9082,5.5,11c0.7941-1.0918,3.871-5.1614,3.871-7.1466C9.371,1.0742,7.2866-0.0176,5.5-0.0176z"/></svg>';
             ...
             return (<Marker
              ...
              >
                <Tooltip>
                  ...
                  <h6>{a?.nama?.usahas[0]?.alamat}</h6><br />
                </Tooltip>
              </Marker>)
            })}
          ...
          {list &&
            list?.map((a, i) => {
              ...
              return (a?.anggota?.map((s, d) => {
                var svg = '<svg width="'+ukuranmarker+'" height="'+ukuranmarker+'"><rect width="'+(ukuranmarker)+'" height="'+(ukuranmarker)+'" fill=' + color + ' fill-opacity="0.6" /></svg>';
